...
public class Model {
	...
	public Model(String file) {
		this.grid = InputData.readFile("level0.txt");
	...
	public HashSet<Rule> generateRules(Cell[][] grid) {
		...
		for (int j = 0; j < grid[0].length; j++) {
			for (int i = 0; i < grid.length; i++) {
				...
				if (i < grid.length - 2) {
					blockX1 = grid[i + 1][j].getBlock(0);
					blockX2 = grid[i + 2][j].getBlock(0);
				}
				if (j < grid[0].length - 2) {
					blockY1 = grid[i][j + 1].getBlock(0);
					blockY2 = grid[i][j + 2].getBlock(0);
				}
